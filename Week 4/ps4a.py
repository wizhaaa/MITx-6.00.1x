...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while calculateHandlen(handCopy) != 0:
        ...
        # Ask user for input
        word = input('Enter a word, or a "." to indicate that you are finsihed: ')
        ...
        # Otherwise (the input is not a single period):
            ...
            # Otherwise (the word is valid):
                ...
                print(str(word), " earned ", str(getWordScore(word,n)), " Total: ", str(totScore), " points")
                # Update the hand 
                handCopy = updateHand(handCopy, word)

    # Game is over (user entered a '.' or ran out of letters), so tell user the total score
    if calculateHandlen(handCopy) == 0 or word == "." :
